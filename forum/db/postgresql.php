    ...
    switch($table){
      case "main":
        $SQL="CREATE TABLE $table_name (
                  ...
                  closed int2 DEFAULT '0' NOT NULL,
        ...
        if(!$q->error()){
          ...
          $SQL="CREATE TABLE ".$table_name."_bodies (
                  ...
                  thread INT4 DEFAULT '0' NOT NULL,
		  CONSTRAINT ".$table_name."_bodiespri_key PRIMARY KEY(id))";
          $q->query($DB, $SQL);
          if(!$q->error()){
      ...
      case "forums":
        $SQL = "CREATE TABLE ".$table_name." (
                  ...
                  required_level int4 DEFAULT '0' NOT NULL,
                  permissions int4 DEFAULT '0' NOT NULL,
                  allow_edit int2 DEFAULT '1' NOT NULL,
                  allow_langsel int2 DEFAULT '0' NOT NULL,
                  displayflag int2 DEFAULT '0' NOT NULL
                  )";
        $q->query($DB, $SQL);
        if(!$q->error()) {
      ...
      case "auth":
        $SQL="CREATE TABLE ".$table_name."_auth (
                   ...
                   hide_email int2 DEFAULT '0' NOT NULL,
                   permission_level int4 DEFAULT '0' NOT NULL,
                   max_group_permission_level int4 DEFAULT '0' NOT NULL,
                   lang varchar(50) DEFAULT '' NOT NULL,
                   password_tmp varchar(50) DEFAULT '' NOT NULL,
		   combined_token varchar(50) DEFAULT '' NOT NULL,
                   CONSTRAINT ".$table_name."_auth_pri_key PRIMARY KEY (id))";
        $q->query($DB, $SQL);
        $SQL="CREATE INDEX name_".$table_name."_auth_ukey ON ".$table_name."_auth (name)";
        $q->query($DB, $SQL);
        $SQL="CREATE UNIQUE INDEX username_".$table_name."_auth_ukey ON ".$table_name."_auth (username)";
        $q->query($DB, $SQL);
        $SQL="CREATE INDEX password_".$table_name."_auth_key ON ".$table_name."_auth (password)";
        $q->query($DB, $SQL);
        break;
      case "moderators":
        // moderator xref
        $SQL="CREATE TABLE ".$table_name."_moderators (
                  ...
                  CONSTRAINT ".$table_name."_moderators_pri_key PRIMARY KEY(user_id,forum_id))";
        $q->query($DB, $SQL);
        $SQL="CREATE INDEX forum_id_".$table_name."_moderators_key ON ".$table_name."_moderators (forum_id)";
        $q->query($DB, $SQL);
        $ret=$q->error();
        return $ret;
        break;
      case "forum2group":
        // forum2group xref
        $sSQL="CREATE TABLE ".$table_name."_forum2group (
          forum_id int DEFAULT 0 NOT NULL,
          group_id int DEFAULT 0 NOT NULL,
          CONSTRAINT ".$table_name."_forum2group_pri_key PRIMARY KEY (forum_id,group_id))";
        $q->query($DB, $sSQL);
        $ret=$q->error();
        return $ret;
        break;
      case "groups":
        // groups
        $sSQL="CREATE TABLE ".$table_name."_groups (
          id SERIAL,
          name varchar(255) DEFAULT '' NOT NULL,
          permission_level int DEFAULT 0 NOT NULL,
          CONSTRAINT ".$table_name."_groups_pri_key PRIMARY KEY (id)
        )";
        $q->query($DB, $sSQL);
        $ret=$q->error();
        return $ret;
      ...
      case "user2group":
        // user2group xref
        $sSQL="CREATE TABLE ".$table_name."_user2group (
          user_id int DEFAULT 0 NOT NULL,
          group_id int DEFAULT 0 NOT NULL,
          CONSTRAINT ".$table_name."_user2group_pri_key PRIMARY KEY (user_id,group_id)
        )";
        $q->query($DB, $sSQL);
        $ret=$q->error();
        return $ret;
      ...
      case "attachments":
        $SQL="CREATE TABLE ".$table_name." (
                  ...
                  CONSTRAINT ".$table_name."_attachments_pri_key PRIMARY KEY(id, message_id))";
        $q->query($DB, $SQL);
        $SQL="CREATE INDEX message_id_".$table_name."_attachments_key ON ".$table_name."_attachments (message_id)";
        $q->query($DB, $SQL);
        break;
    }
  }
