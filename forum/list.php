...
////////////////////////////////////////////////////////////////////////////////
  ...
  if ($ForumMultiLevel==2) {
    ...
    $groupbyadd='';
        ...
        $groupbyadd=', modifystamp';
    }
    if($$phcollapse==0){
      $SQL = "SELECT thread, modifystamp FROM $ForumTableName WHERE approved='Y' $myflag GROUP BY thread$groupbyadd ORDER BY modifystamp desc, thread desc".$limit;
    } else {
      if($DB->type=='mysql'){
      ...
      $SQL = "SELECT thread, modifystamp, count(id) AS tcount, $convfunc(modifystamp) AS latest, max(id) as maxid FROM $ForumTableName WHERE approved='Y' $myflag GROUP BY thread$groupbyadd ORDER BY modifystamp desc, thread desc".$limit;
    }
  ...
  // get userinfo for the selected posts
  if(@is_array($ids)){
...
<tr>
  <td width="60%" nowrap="nowrap" <?php echo bgcolor($ForumNavColor); ?>><?php echo $TopLeftNav; ?></td>
...
<table width="<?php echo $ForumTableWidth; ?>" cellspacing="0" cellpadding="3" border="0">
<tr>
  <td width="60%" nowrap="nowrap" <?php echo bgcolor($ForumNavColor); ?>><?php echo $LowLeftNav; ?></td>
