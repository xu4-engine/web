<?php
    ...
    settype($start_num, "integer");

    if(empty($date)) $date=30;
    if(empty($start_num)) $start_num=1;
...
<?php
        if($rows>0){
            ...
            While(is_array($message)){
                ...
                echo "<dl><dt><strong>$count. </strong><a href=\"$read_page.$ext?f=$num&i=".$message["id"]."&t=".$message["thread"]."$GetVars#reply_".$message['id']."\"><strong>".chop($message["subject"])."</strong></a> - ".chop($message["author"])."<br />\n<dd>";
