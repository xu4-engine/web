...
reset($argv);
foreach($argv as $arg) {
    if (preg_match("/^forum=([0-9]+)/", $arg, $regs)) {
        $num = $regs[1];
    } elseif (preg_match("/^path=(.+)/", $arg, $regs)) {
        $phorum_path = $regs[1];
    } elseif (strstr("verbose", $arg)) {
        $verbose = true;
    } 
} 
...
if (@preg_match("/([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)/", $eHeaders["received"], $regs)) {
    $ip = $regs[0];
} else {
    $ip = "PhorumMail";
} 
...
if (empty($eHeaders["from"])) {
    ...
    if (preg_match('/"([^"]+)" <([^>]+)>/', $eHeaders["from"], $regs)) {
        $author = $regs[1];
        $email = $regs[2];
    } else if (preg_match('/([^<]+)<([^>]+)>/', $eHeaders["from"], $regs)) {
        $author = trim($regs[1]);
        $email = $regs[2];
...
if (empty($eHeaders["subject"])) {
    ...
    $subject=preg_replace("/\[$ForumEmailTag\] */", "", $subject);
}
...
$inreplyto = '';
if (@preg_match('/^(<[^>]+>)/', $eHeaders["in-reply-to"], $regs)) {
    $inreplyto = $regs[1];
} else if (@preg_match('/^(<[^>]+>)/', $eHeaders["references"], $regs)) {
    $inreplyto = $regs[1];
} 
