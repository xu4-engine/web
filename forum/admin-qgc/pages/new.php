...
<tr>
  ...
  <td valign="middle">
    <select name="displayflag">
      <option value="0" <?PHP if($displayflag==0) echo "selected"; ?>>Show only in selected folder</option>
      <option value="1" <?PHP if($displayflag==1) echo "selected"; ?>>Show in each folder</option>
      <option value="2" <?PHP if($displayflag==2) echo "selected"; ?>>Show also in each subfolder</option>
    </select>
  </td>
</tr>
<?PHP } ?>
...
<option value="1" <?php if($allow_edit==1) echo "selected"; ?>>Yes</option>
<option value="0" <?php if(isset($allow_edit) && $allow_edit==0) echo "selected"; ?>>No</option>
</select></td>
</tr>
<tr>
...
<?php if($AllowAttachments){ ?>
  ...
  <?php
    if(!empty($AttachmentFileTypes)){
      ...
      echo "There are no file types listed in the<br /><a HREF=\"?page=attachments\">Phorum Setup</a>.  Therefore all will<br />be allowed.";
    }
  ?>
  </td>
<?php } ?>
