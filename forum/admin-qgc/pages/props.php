...
check_security($num);
if(!$ModEditProps && empty($PHORUM["admin_user"]["forums"][0])) {
      return;
}
    /* Forum Properties*/
...
<?PHP if(!$forum['folder']){ ?>
<tr>
  <th valign="middle">Display option:</th>
  <td valign="middle">
    <select name="displayflag">
      <option value="0" <?PHP if($forum['displayflag']==0) echo "selected"; ?>>Show only in selected folder</option>
      <option value="1" <?PHP if($forum['displayflag']==1) echo "selected"; ?>>Show in each folder</option>
      <option value="2" <?PHP if($forum['displayflag']==2) echo "selected"; ?>>Show also in each subfolder</option>
    </select>
  </td>
</tr>
<?PHP } ?>
<tr>
  <th valign="middle">Userselection of Language:</th>
  <td valign="middle"><input type="checkbox" name="allow_langsel" value="1" <?php echo $forum['allow_langsel']==1 ? "checked=\"checked\"" : "";?>></td>
</tr>
<tr>
...
<tr>
  <th valign="middle">Edit Posts:</th>
  <td valign="middle"><select name="allow_edit" class=big>
<option value="0" <?php if($forum["allow_edit"]==0) echo "selected"; ?>>No</option>
<option value="1" <?php if($forum["allow_edit"]==1) echo "selected"; ?>>Yes</option>
</select></td>
</tr>
<tr>
...
<tr>
  <th valign="middle">Show Users IP/Host:<div style="font-size: 9px; color: Red;">Note: admins/moderators will always see them.</div></th>
...
<option value="0" <?php if($forum["html"]==0 || $forum["html"]=="") echo "selected"; ?>>Off</option>
...
<tr>
  <td colspan="2" valign="middle">Attention! Please read docs/usage.txt first!</td>
</tr>
<tr>
  <th valign="middle">Permission Level - Security:</th>
  <td valign="middle">
<select name="permissions">
<option value="0" <?php if($forum["permissions"]==0) echo "selected"; ?>>no groups or levels</option>
<option value="1" <?php if($forum["permissions"]==1) echo "selected"; ?>>groups only</option>
<option value="2" <?php if($forum["permissions"]==2) echo "selected"; ?>>levels only</option>
<option value="3" <?php if($forum["permissions"]==3) echo "selected"; ?>>groups OR levels (either group or level matches)</option>
<option value="4" <?php if($forum["permissions"]==4) echo "selected"; ?>>groups AND levels (group and level matches)</option>
</select>
</td>
</tr>
<tr>
  <th valign="middle">Required Permission-Level:</th>
  <td valign="middle"><input type="text" name="required_level" value="<?php echo $forum['required_level'];?>"></td>
</tr>
<tr>
  <th valign="middle">Allowed Groups:</th>
  <td valign="middle">Groups which are granted to access this forum</td>
</tr>
<tr>
<th>&nbsp;</th>
<td>
...
<?php
$SQL="SELECT b.id,b.name FROM ".$PHORUM["forum2group_table"]." as a, ".$PHORUM["group_table"]." as b WHERE a.forum_id=$num AND a.group_id=b.id";
$q->query($DB,$SQL);
while($row=$q->getrow()) {
	print "<tr>";
	print "<td valign=\"middle\">".$row['name']."</td>";
	print "<td valign=\"middle\"><a href=\"$myname?page=props&action=groups&subaction=removegroup&gid=".$row["id"]."&fid=$num\">remove from forum</a></td>";
	print "</tr>";
}
...
<tr>
  <th valign="middle" colspan="2"><a href="<?php echo "$myname?fid=$num&page=searches";?>">Add a Group</a></th>
</tr>
