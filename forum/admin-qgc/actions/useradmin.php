...
switch($subaction) {
    ...
    case 'save_user':
	    ...
	    // its time to del the modcache
	    phorum_del_modcache();
          if ($permission_level=='') $permission_level=0;
            if(!empty($uid)){
                $sSQL="UPDATE $pho_main"."_auth SET username='$username', name='$name', $pass email='$email', webpage='$webpage', image='$image', signature='$signature', icq='$icq', yahoo='$yahoo', aol='$aol', msn='$msn', jabber='$jabber', permission_level='$permission_level' WHERE id='$uid'";
                $q->query($DB, $sSQL);
