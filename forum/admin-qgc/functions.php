<?php
  ...
  // deletes the modcachwe (for refresh)
  function phorum_del_modcache() {
    global $PHORUM;
    @unlink($PHORUM["moderator_cache"]);
  }

  // deletes the groupcache (for refresh)
  function phorum_del_groupcache() {
    global $PHORUM;
    @unlink($PHORUM["group_cache"]);
  }

  // calls the two del-functions above
  function phorum_del_caches() {
    phorum_del_modcache();
  ...
  function writefile($forum='', $main=false) {
    global $PHORUM, $DB, $q, $inf_path,$inf_file,$inf_back,$down,$dbName,$dbUser,$dbPass,$dbServer,$DefaultDisplay,$DefaultEmail,$ReplyLocation,$PhorumMailCode,$SortForums,$Password,$UseCookies,$ShowAttachments, $ConfirmRegister;
    ...
    global $ModEditProps, $VisCreateAcc, $UserModifyProf;
    ...
    if($main!=false || $forum=='all' || $forum==''){
      ...
      $data.="  \$PHORUM['ReplyLocation']='$ReplyLocation';\n";
      $data.="  \$PHORUM['PhorumMailCode']='$PhorumMailCode';\n";
      $data.="  \$PHORUM['UseCookies']='$UseCookies';\n";
      $data.="  \$PHORUM['ConfirmRegister']='$ConfirmRegister';\n";
      ...
      $data.="  \$PHORUM['ShowAttachments']='$ShowAttachments';\n";
      $data.="\n";
      $data.="  \$PHORUM['ext']='$ext';\n";
      ...
      $data.="  \$PHORUM['ModEditProps']='$ModEditProps';\n";
      $data.="  \$PHORUM['VisCreateAcc']='$VisCreateAcc';\n";
      $data.="  \$PHORUM['UserModifyProf']='$UserModifyProf';\n";
      $data.="\n";
    ...
    if($forum!=''){
      ...
      While(isset($rec->id)){
        ...
        if(!$rec->folder){
          $data.="  \$PHORUM['ForumAllowLang']='$rec->allow_langsel';\n";
          $data.="  \$PHORUM['ForumAllowEdit']='$rec->allow_edit';\n";
          ...
          $data.="  \$PHORUM['ForumPermissions']='$rec->permissions';\n";
          $data.="  \$PHORUM['ForumRequiredLevel']='$rec->required_level';\n";
          $data.="  \$PHORUM['ForumCheckDisp']='$rec->displayflag';\n";
          // get the allowed groups
          $gSQL="SELECT group_id FROM ".$PHORUM["forum2group_table"]." WHERE forum_id=".$rec->id;
          $r=new query($DB,$gSQL);
      while($grec=$r->getrow()) {
        $data.="  \$PHORUM['ForumAllowedGroup'][]='".$grec['group_id']."';\n";
      }
        }
  ...
  // returns the value of the allowed html-tags
  ...
  // small function for creating a message-string for output
  ...
  function DropForum($f, $table) {
    ...
    if(strlen($AttachmentDir) > 0 && strlen($table) > 0) {
  ...
  // deletes a folder and all forums/folders in it (recursively)
  ...
  // checks if the user is allowed to go into the admin-area
  function check_security($reqforum=0)
  {
    global $PHORUM, $f;
    if(!empty($DB->connect_id) && empty($PHORUM["admin_user"]["forums"][$reqforum]) && empty($PHORUM["admin_user"]["forums"][0])){
        header("Location: $PHP_SELF");
        exit();
    }
  }

  // returns the permissions of a given file (unix only???)
function check_fileperms($str, $name){
  global $lAdmin_Perm_Ok, $lAdmin_Perm_Bad;
    $perms=fileperms($str);
    $dir=0;
    if(is_dir($str)) {
    $dir=1;
    ...
    if(@fclose(@fopen($str, 'a'))) {
        if($dir) {
           @unlink($str);
        }
        return $name.$lAdmin_Perm_Ok;
    } else {
        return $name.$lAdmin_Perm_Bad;
    }
  }
?>
